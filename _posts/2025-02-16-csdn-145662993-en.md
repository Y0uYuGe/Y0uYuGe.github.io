---
layout: "single"
title: "2025 Srdnlen CTF -- A child's dream (a Nintendo game pwn)"
date: "2025-02-16 23:19:17 +0800"
lang: en
permalink: "/en/CTF与Pwn/csdn-145662993/"
categories:
  - "CTF与Pwn"
tags:
  - "pwn"
  - "CTF"
series: "CTF Writeup"
excerpt: "While messing around, I noticed that earlier button presses caused different results for the subsequent R press. I guessed they might be related, so I planned to set a breakpoint to watch the contents of 0x7f0000. After testing, I found that the up/down buttons affect the return address — something like up button +1 and down button -1 — but the bit position being modified differs, seemingly related to left/right movement. I found that DEEE uses it. And only that place — maybe it outputs the flag. It should belong to the DEDF function, since it's separated from the code above by an RTL. When pressing R, there is no JSL $00E069 — it goes directly past it, and there's no RTL back to BRA $00CD5E either. I found the return address is 1 more than the return address saved on the stack — maybe that's just a quirk of this assembly."
source: "CSDN"
...
Yep, I'm a clickbaiter :)

xiao✌ later told me the source code was available.
...
At first, seeing an SFC file confused me a bit. After searching, a bunch of inexplicable things came up and I couldn't figure it out. Only after reading the writeup did I realize it's a Nintendo game.
...
The hint says to use the mesen emulator to run it; see the reference links below for download and usage.
...
Or see the official documentation for details.
[https://www.mesen.ca/docs/](https://www.mesen.ca/docs/)

Writing an FC/SFC emulator
...
w d: move left/right; u: end the game; i: crash or freeze; L: pause

mesen provides simulated keyboard keys corresponding to the buttons of different games.
![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-001-54bdc57ed2.png)

# Vulnerability

At first I had no idea where to start. I couldn't reverse it, so I had to debug to find the input handling point and where the game starts running.

Trying to find where input is handled: while debugging, repeatedly continuing and running, I found it pauses here for a while. Judging from the instruction's meaning, it should be waiting for input to generate an interrupt.

![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-002-73b4a91cc0.png)
Debugging by inputting directly in the game doesn't work at all, because its output per cycle is too fast — before I could type anything it already came back. Fortunately, input can be sent through the debugger's controller.

![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-003-5351550599.png)

Then I randomly tried the various buttons and found that pressing the R key froze it directly, and that was that...

~~After reading the writeup, it says there's a flag inside and you just need to print it out yourself. Didn't think of that — I'm too noob~~

Then I searched and found a flag at D658.
I found that the DEEE function uses this address. And only that place — maybe it outputs the flag. It should belong to the DEDF function, since it's separated from the code above by an RTL.
![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-004-146de1fcb3.png)

So the question is how to call the DEDF function.

# Processing logic

Where does execution go after the WAI interrupt? Because I found that after WAI, clicking step had no response.
I wanted to see how to enter the interrupt handler — maybe that's where my input is processed — because I noticed that from WAI to the next instruction, the game advanced one frame.
Searching online for how mesen enters the interrupt handler, I learned it enters NMI.

Then through this
![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-005-05dc2a7ccc.png)
I entered the interrupt handler.

But I seemed to be wrong — maybe there's latency between the debugger and actual execution? Because there's no input handling in there.

Then I searched online, suffered from the lack of Chinese resources, found nothing useful, and looked at the writeup.
...
Debugging and looking at memory: the value of the pressed button is in 4218. Then I found that after WAI, when reaching the next instruction, the value of 4218 indeed changed, and I learned the values corresponding to each button.
...
Then how to handle it? ~~Still don't know, I'm trash~~
We should need to use the values of these input buttons. The writeup says to search for what uses the address where the button value is stored. There is one — an instruction at address 208 in NMI.

> Found that below, the button value gets stored at address 39 for later use.
> ![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-006-7c32cc4c7d.png)

But then it returns, going back to the instruction after the original WAI. Not useful.

The writeup mentions looking at references to addresses holding the same content — also not useful. ~~Maybe I'm just too noob~~

Watashi, is it really going to end like this?

No — rather than caring about how input is processed, I figured I should first consider how the R key causes the crash.

Then, stepping through bit by bit, I learned the following.

The important logic of the function is as follows. I stepped through it instruction by instruction — my eyes were sore. I really admire those reverse-engineering experts.

This enters a loop that continuously processes input values, then renders a frame and moves.
...
Comparing with other keys, I found that when pressing R there is no JSL $00E069, it goes directly to RTL, and it also doesn't return to BRA $00CD5E.
![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-007-26a2220e28.png)

But when starting at 00C0AC, the return address is normal at this point.![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-008-c486b90905.png)
I found the actual return address is 1 more than the return address saved on the stack, and in the memory viewer, the starting byte of the return address seems to be separated from the current stack pointer by one byte. Maybe that's just a quirk of this assembly.

So the problem is in this segment:
`00C0AC [0040AC]  3B             TSC to 00C8EA [0048EA]  6B             RTL`
It not only changed the return address but also never entered 00E069.

Then I set a conditional breakpoint to automatically break when the return address on the stack changes.

![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-009-a6764edfac.png)
Found it entered `00C3F7 [0043F7]  22 01 83 01    JSL $018301`

`                     --------sub start-------- It changed after 018301 [008301]  A3 04          LDA $04,S`
After more debugging I found it was caused by `018316 [008316]  20 28 00       JSR $0028`, which jumps to 0028 to execute.
...
Using AI to reverse it, the rough logic is as follows:
...
So the goal is how to modify 0x7f0000.

While messing around, I noticed that earlier button presses caused different results for the subsequent R press. I guessed they might be related, so I planned to set a breakpoint to watch the contents of 0x7f0000. After testing, I found that the up/down buttons affect the return address — something like up button +1 and down button -1 — but the bit position being modified differs, seemingly related to left/right movement. Also, the ball must not fall, otherwise it gets cleared to zero.

I didn't fully reverse this part; I'll look at it again when I have time.

> Addition:
> I wanted to see how each button is handled. I think it should be in the earlier part of the code logic, because in this segment the R key behaves differently from other keys, and I found that other buttons don't execute
> 00C3F7 [0043F7]  22 01 83 01    JSL $018301
> Looking a bit further up, there happens to be an R button value 0x10
> ![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-011-4c747ab6e2.png)
> Combined with the bunch of ANDs with various button values appearing earlier, this should be where button values are processed before entering the corresponding function. So the code from `00C0AC [0040AC]  3B             TSC to 00C8EA [0048EA]  6B             RTL` should be what handles input — there are jumps in between because the code is not contiguous.
...
> Fed it to AI for reversing. The core logic of the code revolves around the state of $328B. By extracting specific bit flags from $328B (such as $0080, $0100,
> $0200, $0400, $0800, $1000, $2000, etc.), the program determines the current state.
> Depending on which bit flag is set, the program dispatches to different processing logic branches. For example, if the $0080 bit is set, it jumps to a specific processing logic; otherwise it continues checking other bits.
> I searched for 328B, and there's one near the start.
> ![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-012-4bd2d011f4.png)
> Here the value at address 39 is stored into 328B, and the content at address 39 is exactly the button value stored earlier. It all connects. Satisfying.
>
> Next is reversing how each button affects the position at 0x7F0000.
> The buttons that have an effect are as follows:
>
> A button
>
> ```
...
Can't reverse it at all. With a memory watch breakpoint, the logic is as follows: it indirectly dereferences address 00 to get its value, then +1, then puts it back.
![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-013-f4b67105c0.png)
The content stored at address 00 is obtained as follows:
![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-014-55a2296a52.png)

Below is the offset: the offset stored at position 3292 is added to the original position value (both are zero though, because they were assigned zero earlier), then put back to the original position.

![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-015-9050bbb6f8.png)

The offset is stored at 3292. Debugged with breakpoints to see how it changes, and found that left/right movement changes it.

Directly searched for instructions containing 3292.
Found it in the left-move code: it loads the content at position 3292, stores it at address 00, loads it into A, adds -1, and stores it back to 3292.
...
So the final goal is to turn it into DEDF, while also making sure the ball doesn't fall.

Because it automatically adds 1, the target is DEDE: first subtract 1 to get to 0xff, then keep subtracting down to 0xDE. It really tests your hand speed.

![在这里插入图片描述](/images/csdn/2025/csdn-145662993/image-018-abc3acf766.png)

The writeup says using the A button as an assist changes it by 0x10 at a time — that might be easier, but I didn't think of it. Still, I feel it's doable without that, just requires fast hands.

But this instruction is at
...
Personally I feel my reverse-engineering skills need improvement; it mostly relied on reversing and guessing based on problem-solving intuition.
{% endraw %}
