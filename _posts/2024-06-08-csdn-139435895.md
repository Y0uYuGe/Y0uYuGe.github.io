...
tags:
  ...
  - "Pwn"
series: "CTF Writeup"
excerpt: "add checks the index, then allocates a fixed 0x40-sized chunk for that index and copies the content of whatcon into it. delete can be used 10 times; it checks the index range and whether the chunk at the corresponding index is NULL, but the pointer is not cleared after free. show can be used 3 times; it also checks the index range and whether the chunk at the corresponding index is NULL, and it prints the chunk content when neither whatsthis == '\\xFF' nor whatsize == 48 is satisfied. At the time I had no idea protobuf was being used, and the extraction tools failed to extract it either. I had solved too few challenges, and I failed to recognize many key structures as protobuf."
source: "CSDN"
original_url: "https://blog.csdn.net/llovewuzhengzi/article/details/139435895"
csdn_article_id: "139435895"
migrated: true
permalink: /posts/csdn-139435895/
redirect_from:
  - "/en/CTF与Pwn/csdn-139435895/"
  - "/CTF与Pwn/csdn-139435895/"
---
...
# protobuf
At the time, I had no idea that protobuf was being used, and the extraction tools failed to extract it either. I had simply solved too few challenges, and I failed to recognize many key structures as protobuf.
Next time, I can search for the magic header of the ProtobufCMessageDescriptor struct (usually 0x28AAEEF9) to locate the ProtobufCMessageDescriptor.
![Image](/images/csdn/2024/csdn-139435895/image-001-dc709b3ee3.png)
![Image](/images/csdn/2024/csdn-139435895/image-002-925901461d.png)
...
```
Then, follow ProtobufCFieldDescriptor to find the contents of each field inside.
...
```
Definitions of the related fields
...
```
Restore the original structure based on these four fields: `const char		*name;
uint32_t		id;
ProtobufCLabel		label;
ProtobufCType		type;`
...
From the name field of ProtobufCMessageDescriptor, we learn that this message is named `heybro`. Then, from ProtobufCFieldDescriptor's `const char		*name;
uint32_t		id;
ProtobufCLabel		label;
ProtobufCType		type;` we get the name, id, label, and type of each field.
...
# Approach
add checks the index, then allocates a fixed 0x40-sized chunk for that index and copies the content of whatcon into it. delete can be used 10 times; it checks the index range and whether the chunk at the corresponding index is NULL, but the pointer is not cleared after free. show can be used 3 times; it also checks the index range and whether the chunk at the corresponding index is NULL, and prints the chunk content when neither whatsthis == '\xFF' nor whatsize == 48 is satisfied.
There is a double free and a show-after-free.
1. During add, a 0x50 chunk and a chunk with the same length as data are allocated first; both are taken from the unsorted bin. The content is first written into the chunk that has the same length as data, and finally memcpy writes the data chunk into the allocated 0x40 chunk. Here, the residual libc data can be used to leak an address.
![Image](/images/csdn/2024/csdn-139435895/image-003-d0d9caa6ca.png)
![Image](/images/csdn/2024/csdn-139435895/image-004-5a4b70462b.png)
2. Leaking the heap address: a direct leak is useless, because due to the behavior of memcpy and the fact that content must be written, the residual heap address gets overwritten. Just free into tcache to leak it.
![Image](/images/csdn/2024/csdn-139435895/image-005-3c2db48857.png)
3. Construct an arbitrary write. However, since delete is limited to exactly 10 uses, we only get one arbitrary write. But at this point there are many other chunks in the tcache bins, so we can write to the tcache bin to achieve multiple arbitrary writes. To leak the stack address, we can arbitrary-write stdout and then leak the stack address.
![Image](/images/csdn/2024/csdn-139435895/image-006-0e4882f232.png)
 ![Image](/images/csdn/2024/csdn-139435895/image-007-2b6d50e230.png)
 IO_2_1_stdout: _flags = 0xFBAD1800, then set the following three read parameters to 0, and the range between write_base and write_ptr becomes the address range we want to output.
 ![Image](/images/csdn/2024/csdn-139435895/image-008-9797faaf54.png)
4. malloc allocates into the tcache bin, then modify the tcache bin again so that allocations can land on the stack.

![Image](/images/csdn/2024/csdn-139435895/image-009-c6e962957c.png)
...
![Image](/images/csdn/2024/csdn-139435895/image-010-109d118c5c.png)
{% endraw %}
