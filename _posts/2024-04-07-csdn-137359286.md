---
layout: "single"
title: "Reading the Source Code of a Chinese Address Tokenizer (jiedi)"
...
excerpt: "This code implements an address segmenter based on a Hidden Markov Model (HMM). Hidden state definitions: 'pB', 'pM', 'pE' represent the beginning, middle, and end characters of a province respectively; 'cB', 'cM', 'cE' represent the beginning, middle, and end characters of a city; 'aB', 'aM', 'aE' represent the beginning, middle, and end characters of a district; 'dB', 'dM', 'dE' represent the beginning, middle, and end characters of a detailed address. The Tokenizer class loads the relevant probability parameters of the Hidden Markov Model at initialization, including start probabilities, transition probabilities, and emission probabilities, and implements viterbi."
source: "CSDN"
original_url: "https://blog.csdn.net/llovewuzhengzi/article/details/137359286"
csdn_article_id: "137359286"
migrated: true
permalink: /posts/csdn-137359286/
redirect_from:
  - "/en/NLP/csdn-137359286/"
  - "/NLP/csdn-137359286/"
---
...
# structure
Click the Structure button on the left to enter the Structure view. From Structure we can see how many global variables, functions, and classes the current code file contains, as well as how many member variables and member functions each class has.

The V icon represents a global variable, the pink f icon represents a regular function, the f icon with a small red triangle in the upper-left corner represents an inline function, the C icon represents a class, the m icon inside a class represents a member function, and the f icon represents a member variable. Clicking an icon jumps to the corresponding code.

# .p Files
Files with the `.p` extension usually represent binary data files serialized using Python's `pickle` module.

Specifically:

1. `pickle` is a module in the Python standard library used for object serialization and deserialization. It can convert Python objects into byte streams and save them to files.

2. Typically, the `pickle.dump()` function is used to save Python objects to files with the `.p` extension. This makes it easy to persistently store complex Python objects and, when needed, use the `pickle.load()` function to read and restore the objects from the file.

3. Besides the `.p` extension, you may sometimes see files with `.pkl` or `.pickle` extensions; they all represent binary data files in pickle format.


#  `pd.read_excel` 

`pd.read_excel` is a function in the Pandas library for reading Excel files. It can read Excel files in .xls and .xlsx formats and convert them into Pandas DataFrame objects.

The main parameters of this function are:

1. `file`: the path or filename of the Excel file to read. It can be a local file path or a network URL.

2. `sheet_name`: specifies the name or index of the worksheet to read. The first worksheet is read by default.

3. `header`: specifies the row number used as the column names; the default is 0, meaning the first row is used as column names.

4. `index_col`: specifies which column to use as the row index.

5. `usecols`: specifies which columns to read; it can be a list of column names or a list of column indices.

6. `dtype`: specifies the data type of each column.

7. `na_values`: specifies which values are treated as missing values.
...
In summary, the `pd.read_excel` function provides a flexible way to read Excel files and convert them into Pandas DataFrame objects, facilitating subsequent data processing and analysis.

#  enumerate
The enumerate() function is a built-in Python function that can convert an iterable object (such as a list, string, etc.) into an enumerate object.
Each element in the enumerate object is a tuple containing the element's index and value.



# Mind Map
[Source Code Repository Link](https://github.com/FULLK/Address-segmentation-learning-and-practice/tree/main/address_cut-master)

![Image](/images/csdn/2024/csdn-137359286/image-001-000e536100.png)
...
```
# Summary
This code implements an address segmenter based on a Hidden Markov Model (HMM).

1. **Hidden State Definitions**:
   - `'pB'`, `'pM'`, `'pE'`: represent the beginning, middle, and end characters of a province respectively.
   - `'cB'`, `'cM'`, `'cE'`: represent the beginning, middle, and end characters of a city respectively.
   - `'aB'`, `'aM'`, `'aE'`: represent the beginning, middle, and end characters of a district respectively.
   - `'dB'`, `'dM'`, `'dE'`: represent the beginning, middle, and end characters of a detailed address respectively.

2. **Tokenizer Class**:
   - Loads the relevant probability parameters of the Hidden Markov Model at initialization, including start probabilities, transition probabilities, and emission probabilities.
   - Implements the `viterbi` algorithm, used to find the most probable hidden state sequence for an input address.
   - Implements the `get_emit_p` and `get_trans_p` functions, used to obtain emission probabilities and transition probabilities respectively.
   - Implements the `revise_address_cut` function, used to correct the segmented address based on the standard address library.
   - Implements some auxiliary functions for recording elapsed time.

3. **cut Function**:
   - Takes an address string as input and returns the segmented province, city, district, and detailed address.
   - Calls the `viterbi` algorithm in the Tokenizer class to obtain the most probable hidden state sequence.
   - Based on the hidden state sequence, splits the address string into province, city, district, and detailed address.
   - Calls the `revise_address_cut` function to correct the segmentation result based on the standard address library.
   - Returns the segmented province, city, district, and detailed address, along with related probability and timing information.

4. **Main Function**:
   - Reads a test address sample set and segments each address.
   - Saves the segmentation results into the test sample set, and calculates the total time taken and the average time taken.
   - Finally saves the corrected test sample set to an Excel file.

Overall, this code implements an address segmenter based on a Hidden Markov Model that can segment addresses into province, city, district, and detailed address with reasonable accuracy, and provides related probability and timing information.
{% endraw %}
