---
layout: "single"
title: "Building a C Interpreter from Zero to One - CPC Source Code"
...
excerpt: "Precedence climbing means that a pushed operator needs to keep moving upward; whenever an operator with lower precedence than the current peak is encountered, the related operators keep being evaluated until the precedence of the operator at the peak is less than or equal to the operator to be pushed. Note that since the stack is located at high addresses and grows toward low addresses, the value of SP decreases when pushing. What is pushed onto the operator stack now is the - sign, but its precedence is lower than *, so the result of 3*4 is computed first and pushed onto the data stack, and then the - sign is pushed onto the operator stack. Push operands and operators separately, but when an operator with lower precedence than the current stack-top operator appears, evaluate the current stack-top operator with the corresponding operands. PC is the program counter; it holds a memory address that stores the next computer instruction to be executed."
source: "CSDN"
original_url: "https://blog.csdn.net/llovewuzhengzi/article/details/136308052"
csdn_article_id: "136308052"
migrated: true
permalink: /posts/csdn-136308052/
redirect_from:
  - "/en/项目记录/csdn-136308052/"
  - "/项目记录/csdn-136308052/"
---
...
# Framework
Industrial-grade compilers are divided into a front end and a back end

Front end (parser): source code -> intermediate code (IR or virtual machine instructions)
- Lexical analysis (tokenize): tells the computer what each word means, splitting it into many tokens  
- Syntax analysis: parses statements, then generates intermediate code or an abstract syntax tree

Back end: intermediate code -> target code
- optimizer (the hardest and most core part): intermediate code -> intermediate code  
- codegenerate: intermediate code -> final target code (for a physical architecture it gets translated to the corresponding X86 architecture, etc.; for a virtual machine it gets translated to bytecode)  LLVM translates IR bytecode into the final target code according to the target platform
 

# Design
- The front end and back end are combined into one, with no intermediate optimization; intermediate code is generated directly and executed by the interpreter (a custom VM)
- Lexical analysis and syntax analysis proceed simultaneously while generating bytecode (variable declarations must come at the beginning)

Computation is based on the register and the top of the stack
- register: pc/sp/bp/ax
- memory: code/data (static variables/literals)/stack
- instruction set: save&load (mem<->register) / arithmetic instructions (arithmetic, bitwise, logic) / jumps (unconditional, conditional, comparison) / native call (IO: printf orw / dynamic memory allocation: malloc/free/memset)


PC is the program counter. It holds a memory address, and that address stores the next computer instruction to be executed.
SP is the pointer register, always pointing to the current top of the stack. Note that since the stack is located at high addresses and grows toward low addresses, the value of SP decreases when pushing.
BP is the base pointer. It is also used to point to certain positions on the stack, and is used when calling functions.
AX is a general-purpose register. In our virtual machine, it is used to store the result after an instruction executes.
# VM Instruction Set
- save/load: imm / lea / lc / li /sc / si /push
- arithmetic: add /sub / mul /div /mod /or /xor /and / shc /shr
- branch jumps: jmp/jz / jnz /call /nvar /darg / re
...
# Allocating Space
First read the file into the allocated space

Then allocate the code segment, data segment, stack segment, and symbol table
...
# Lexical Analysis
The lexical analysis function determines the type based on the scanned character

symbol_ptr is the symbol table pointer

- If a token is composed of letters or digits, it is placed into the symbol table. Here the symbol table is traversed to check whether the same symbol exists; if so, that symbol is returned directly; if not, it is inserted, and the returned token is of the ID type
...
```
- Pure numbers are parsed according to their base, and the returned token is of the number type
- Characters and strings: here char is treated as the num type; in addition, strings may contain escape characters, which are also handled here
...
```
- Operators: except for some symbols that are returned directly, most symbols need to return their corresponding operation type
...
- First, some keywords are placed into the symbol table; the Token of these symbols is set to the value corresponding to the keyword's enum value, that is, the corresponding keyword type
After the keywords starting from OPEN are placed into the symbol table, Class (type), type (return value), and value (the corresponding keyword enum value) are also set; here the Token is the ID type enum value
...
Semantics: the meaning being expressed
Grammar: the collection of syntax
...
```
Use recursive descent to decompose layer by layer down to expressions; expressions are handled through precedence climbing

Here syntax analysis and lexical analysis are actually done together: first lexical analysis, then syntax analysis
- Here the parser first parses the declaration statements, that is, the three kinds `enum_define | vari_define| fun_define`, by first parsing the token to know the type (Enum or Char and Int)

- Enum is `Enum { }  or  Enum ID { }   `
```cpp
...
```
- Parsing the function body. First parse the variable declarations, then there is the statement that allocates the stack frame, then parse the statements, finally add a ret, and then restore the symbol table entries that are Loc (because the function has been parsed)
...
```
- Parsing statements: there are if, while, return, {}, ;, and assignment expression statements, which are then converted into the corresponding bytecode
- For an if statement, if the conditional expression inside () evaluates to 0, jump to the else part; otherwise execute the true part
...
<factor> ::= ( <expr> )
           | Num
```
The above is the recursive descent method for parsing expressions
# Expressions
- statement: what to do — if, while, return  
- expression: evaluation — 2+3, 4*5
...
```
# Precedence Climbing
Push operands and operators separately, but when an operator with lower precedence than the current stack-top operator appears, evaluate the current stack-top operator with the corresponding operands
```bash
3+3*4-5
```
![Image](/images/csdn/2024/csdn-136308052/image-001-71cb313ee0.png)
What is pushed onto the operator stack at this point is the - sign, but its precedence is lower than *, so the result of 3*4 is computed first and pushed onto the data stack, and then the - sign is pushed onto the operator stack

Precedence climbing means the pushed operators need to keep moving upward; whenever an operator with lower precedence than the current peak is encountered, the related operators keep being evaluated until the precedence of the operator at the peak is less than or equal to the operator to be pushed
Precedence

```bash
*++a++=i == 0？2+3：4*5
```
Here ++a is parsed first, then *++a, then a++... I won't demonstrate it in detail; just climb according to the corresponding precedence
- Parsing expressions: `precd` represents the maximum precedence for the current parse. Parsing proceeds in precedence order; unary operators are still parsed normally (except a++), but binary operators are only parsed when their precedence is greater than the current precd (a++ is counted at the precedence level just below ())
```cpp
