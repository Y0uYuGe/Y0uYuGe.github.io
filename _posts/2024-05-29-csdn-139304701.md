---
layout: "single"
title: "2024 Jinglin CTF - MazeCodeV1"
...
tags:
  ...
  - "CTF"
series: "CTF Writeup"
excerpt: "Here /bin/sh is used as the syscall input. As for the instruction part, we don't know the mmap start address, and the opcode part is also hard to bypass. So the key is the read syscall and the execve syscall — think through the basic gadgets and then adapt them to fit the bypass. There is a map string in IDA, and since it is a square map with 42 rows and columns, we derive the final route from the map, and then get suitable instruction bytes based on the instructions the route needs. When using asm, the corresponding assembly instructions must have newline characters, otherwise two instructions connected on one line will cause problems. \"SPL\" usually refers to the register esp (the stack pointer register)"
source: "CSDN"
original_url: "https://blog.csdn.net/llovewuzhengzi/article/details/139304701"
csdn_article_id: "139304701"
migrated: true
permalink: /posts/csdn-139304701/
redirect_from:
  - "/en/CTF与Pwn/csdn-139304701/"
  - "/CTF与Pwn/csdn-139304701/"
---
...
#  Check
![Image](/images/csdn/2024/csdn-139304701/image-001-4e1d709035.png)
...
```
There is a map string in IDA, and since it is a square map with 42 rows and columns, we derive the final route from the map, and then get suitable instruction bytes based on the instructions the route needs
# Approach
1. Find suitable instructions such that the lowest two bits of the bytecode they form can satisfy finally reaching the T position
2. These instructions can getshell
3. Then when the T position is finally reached, these instructions will be called and we getshell

Here /bin/sh is used as the syscall input. As for the instruction part, we don't know the mmap start address, and the opcode part is also hard to bypass. So the key is the read syscall and the execve syscall — think through the basic gadgets and then adapt them to fit the bypass

# One-byte instructions
push pop  xchg  
# Note
When using asm, the corresponding assembly instructions must have newline characters, otherwise two instructions connected on one line will cause problems

![Image](/images/csdn/2024/csdn-139304701/image-002-c59d041c3d.png)
"SPL" usually refers to the register esp (the stack pointer register)

![Image](/images/csdn/2024/csdn-139304701/image-003-954feda36d.png)

![Image](/images/csdn/2024/csdn-139304701/image-004-15a1854c4e.png)
# Attaching the exp from master S1uM4i and friends
