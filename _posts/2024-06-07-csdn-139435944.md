...
tags:
  - "PWN"
series: "CTF Writeup"
excerpt: "We need to allocate a chunk of a non-tcache size that does not come from a bin, so that after being freed it can enter the unsorted bin; then we fill it through the overflow and can leak it. Next, the allocated book_struct must not come from a bin either; since the overflow goes through the content, the content chunk must also not come from a bin, and chaining two of them together lets the content of the first overflow into the content of the second. Then we leak the libc base address. Although there are many useless already-freed chunks interfering, we can simply make the sizes of the heap layout we construct not match the already-freed chunks; as for heaps in layouts we will not use, we just let them still come from a bin."
source: "CSDN"
original_url: "https://blog.csdn.net/llovewuzhengzi/article/details/139435944"
csdn_article_id: "139435944"
migrated: true
permalink: /posts/csdn-139435944/
redirect_from:
  - "/en/CTF与Pwn/csdn-139435944/"
  - "/CTF与Pwn/csdn-139435944/"
---
...
# Sandbox
![Image](/images/csdn/2024/csdn-139435944/image-001-fdae40cf0a.png)

# Protobuf Present
Extract it with a tool
# Reverse Engineering
Really hard to reverse. I located the relevant functions through strings; a map is used here to map an int to a function. It is too hard to reverse properly, so I just located things via strings.
...
##  see_book
Next time, the types of the struct's variables can be determined through the format string.
...
# Approach
edit has an arbitrary-length overflow. Each add allocates multiple chunks (should be 6: 4 for internal data, 1 used as the special-data buffer that receives the input, and 1 for the book struct address; but when breaking on malloc during debugging there are only 5 — not sure why, maybe that function is different, maybe it is not actually an allocation).

delete frees the four heap pointers and then zeroes the corresponding slot in book_chunk_array, but it does not free book_chunk.

But it feels like the edit arbitrary-length overflow alone is enough. However, add, edit, delete, and show all check whether the chunk_array entry for the index is non-zero.

add first base32-decodes the input data, then protobuf-deserializes it, and then base64-decodes the first four fields.

1. We can directly obtain it from freed chunks, making use of the leftover data. The struct that bookstruct lives in has size 0x40, and the remaining four chunks are 0x20.
 A huge number of chunks exist right from the start. To leak the heap address: allocate twice; the fd part of the 7th 0x20 chunk is just the address where the next part resides shifted right by 12 bits, so we can leak the heap address through the tcache bin.
![Image](/images/csdn/2024/csdn-139435944/image-002-ae1b77fc1a.png)
Allocating the 8th 0x20 chunk causes it to look in the fastbin, and once found there, the fastbin chunks get moved into the tcache.
2. Then leak the libc base address. Although many useless already-freed chunks interfere, we can simply make the sizes of the heap layout we construct not match the already-freed chunks; as for heaps in layouts we will not use, we just let them still come from a bin.
![Image](/images/csdn/2024/csdn-139435944/image-003-4e43c2a590.png)
![Image](/images/csdn/2024/csdn-139435944/image-004-b6c9e44f0e.png)
We need to allocate a chunk of a non-tcache size that does not come from a bin, so that after being freed it can enter the unsorted bin; then we fill it through the overflow and can leak it. Next, the allocated book_struct must not come from a bin; since the overflow goes through the content, the content chunk must also not come from a bin. Chaining two of them together lets the content of the first overflow into the content of the second.

3. Achieve an arbitrary address write: overwrite the address of some field of a book_struct structure with stderr, and then we can write the contents of stderr. Again, this requires one book_struct chunk, one content chunk, and the next book_struct.

![Image](/images/csdn/2024/csdn-139435944/image-005-1ee8da5f9e.png)
Since a 0x40 allocation would split off from the freed chunks, we have to eliminate all of them first (just make sure none of them is larger than 0x40), consuming them from the smallbin. Then the allocation at this point will split from the previously freed 0x500 chunk, which also lets us overflow into and modify the book_struct so as to write to stderr and thereby modify stderr.

4.  house of apple 2 + setcontext + orw to read the flag. Since when the function in the tampered vtable is finally executed, rdi is stderr and rdx is wide_data, we craft wide_data+0xa0 to be the position of the ROP chain part on wide_data, and wide_data+0xa8 to be ret.

It seems that write_ptr also needs to be non-zero here.
![Image](/images/csdn/2024/csdn-139435944/image-006-66321f0cb8.png)
...
```
![Image](/images/csdn/2024/csdn-139435944/image-007-c0fc924c79.png)
...
```
![Image](/images/csdn/2024/csdn-139435944/image-008-7ed6da54b0.png)
{% endraw %}
